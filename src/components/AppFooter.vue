<template>
    ...
    <div class="app-footer-container" @click="recorder">
        {{ content }}
    </div>
...
import { convertToMp3, getRecorder } from '@/utils/recorder'
...
methods: {
    async recorder() {
        if (!this.isBegin) {
            this.startRecorder('recording');
            this.content = "录制中..."
            this.isBegin = true;
        } else {
            this.content = "结束录制,GOA思考中..."
            this.stopRecorder('stop');
            const res = await this.getMp3Data();
            if (this.$refs.audioPlayer) {
                this.$refs.audioPlayer.src = res;
   ...
   * */
  async getMp3Data() {
    const mp3Blob = await convertToMp3(recorder.getWAV(), recorder);
