<template>
    <div style="position: relative;height: 350px;">
            ...
            <v-tabs-window v-model="tabs">
                <v-tabs-window-item v-for="(item, key) in buttonManager" :key="key" :value="item.title">
                    <div class="button-container">
                        <FunButton @updateMap="updateMap" @updatePlan="updatePlan" @beginSearch="beginSearch" @updateWeather="updateWeather"
                            v-for="(button, buttonKey) in item.list" :parentKey="item.key" :key="buttonKey"
                            :buttonKey="button" />
                    </div>
                    <!-- <v-card> -->
                    <!-- <v-card-text> -->
        ...
        <!-- <template v-slot:activator="{ on, attrs }"> -->
          <!-- 使用v-bind绑定激活器的属性和事件 -->
          <!-- <v-btn v-bind="attrs" v-on="on" color="surface-variant" text="Open Dialog" variant="flat"></v-btn> -->
        <!-- </template> -->
